...
file='frgPL_20191021_0002_GG1.h5';
...
L=length(measvolt); % number of measurements
...
for p=1:size(M,1) % For all pixel columns
    for q=1:size(M,2) % For all pixel lines
        ...
        Rs(p,q)=X{p,q}(2); % The Rs value is in the second line of X
        C(p,q)=exp(X{p,q}(1)/VT);
%         J01(p,q)=-X{p,q}(3)*C(p,q)/Rs(p,q);
%         J02(p,q)=-X{p,q}(4)*sqrt(C(p,q))/Rs(p,q);
%         Voc1sun(p,q)=
%         VT*log((GG8(4).PLIV(8).image.mean(p,q)-GG8(4).PLIV(2).image.mean(p,q))/C(p,q));
%         % Local voltage Vxy for the measurements close to 1 sun (0.905 A
%         current). Based on the measurement from 
%         % and to open circuit (I close to 0 A)
        Vmpp1sun(p,q)=VT*log(lumstruc(3).netimage(p,q)/C(p,q)); % V
...
for p=pstart:pend
    for q=qstart:qend
        if(~isnan(Rs_final(q,p)) && Rs_final(q,p)~=0)
            ...
            fprintf('NaN or 0 found for indices line %d col %d',p,q);
        end
    end
end
