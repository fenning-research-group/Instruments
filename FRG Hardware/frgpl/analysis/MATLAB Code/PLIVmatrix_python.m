...
% measurements
nb_V = numel(unique(vertcat(datastruc(2:end).set_voltage))); %Number of voltage values (the two first images are correction images at Jsc)
nb_int= numel(unique(vertcat(datastruc(2:end).intensity))); % Number of laser intensity values
...
% end
intarray = vertcat(datastruc(2:end).intensity);
